...
int downloadFileToClient(SOCKET& socket, char* recvBuffer, sockaddr* recvFrom, size_t offset)
{
	...
	if (strcmp(recvBuffer, "notexists") != 0)
	{
		...
		while (currentSize < fileSize)
		{
			...
			{
				Packet dataPortion;
				recievedData = recvfrom(socket, (char*)&dataPortion, sizeof(Packet), 0, recvFrom, &FROMLEN);
				an = dataPortion.sn;
				...
				fwrite(filePart, 1, recievedData - sizeof(int), downloadedFile);
			}
			else
			{
